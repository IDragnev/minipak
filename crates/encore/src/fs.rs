...
impl File {
    ...
    fn raw_open(path: &str, flags: OpenFlags, mode: u64) -> Result<Self, EncoreError> {
        ...
        let fd = unsafe {
            syscall::open(nul_path.as_ptr(), flags, mode)
        };
        if (fd.0 as i64) < 0 {
    ...
    /// Write a whole buffer to this file, or fail. This may involve multiple syscalls.
    pub fn write_all(&mut self, mut buf: &[u8]) -> Result<(), EncoreError> {
        while !buf.is_empty() {
            let written = unsafe {
                syscall::write(self.fd, buf.as_ptr(), buf.len() as u64)
            };
            if (written as i64) == -1 {
    ...
    pub fn len(&self) -> Result<u64, EncoreError> {
        ...
        let ret = unsafe {
            syscall::fstat(self.fd(), stat.as_mut_ptr())
        };
        if ret != 0 {
